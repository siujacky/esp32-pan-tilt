...
| **Position presets** (task #8) | ✅ built + verified (OTA-shipped) | 4 slots per control target, NVS, glide recall; web main-page card + OLED Presets page + `P,<R\|S\|X>,<n>`; CSV 36-43; reboot-persistence + clear verified |
| **Range test + collision watch** (web Test tab + OLED Test page) | ✅ built + verified live | full 0–180 sweep @10°/s, soft limits bypassed by design, total motion lockout, joystick Confirm/Back = physical STOP; LX collision → back off 5° + HALT. Full sweep tracked live on hardware |
| **Cockpit Target row** (Config page) | ✅ built + flashed | stick switches PWM/SERIAL/BOTH without the web page |
| **Seesaw path removed** | ✅ | speculative dead code deleted (−6.5 kB); nulllab mini is the only joystick |
| **10-finding review round 2** | ✅ all fixed + verified + pushed | uniform input guards, per-slot clamps, signed ticks, pin-validation order — see lessonlearn §22 |
